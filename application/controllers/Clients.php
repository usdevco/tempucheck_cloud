...
class Clients extends Clients_controller
{
    ...
    public function index()
    {
        ...
        $data['temp_data'] =  $return = $this->db->where('userid',get_client_user_id())->get('temp_data')->result();
    ...
    {
        $document = $return = $this->db->where('id',$id)->get('temp_data')->row();
        $filename = $document->pdf_file;
        $real_filename = basename($filename, '/pdf/');
        $this->load->helper('download');
        // $data = file_get_contents(base_url('/assets/uploads/pdf/'.$filename));
        $data = file_get_contents($filename);
        force_download($real_filename, $data);
        exit;
    }
