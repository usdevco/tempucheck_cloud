...
class Clients extends Admin_controller
{
    ...
    public function client($id = '')
    {
        ...
        if ($this->input->post() && !$this->input->is_ajax_request()) {
            if ($id == '') {
                ...
                $save_and_add_contact_only = false;
                if (isset($data['save_and_add_contact_only'])) {
                    unset($data['save_and_add_contact_only']);
                    $save_and_add_contact_only = true;
                }
                ...
                if ($id) {
                   ...
                   /* if ($save_and_add_contact == false) {
                    ...
                    if ($save_and_add_contact == true) {
						redirect(admin_url('clients/client/' . $id . '?group=contacts&new_contact=true'));                        
                    } else if ($save_and_add_contact_only == true) {
						redirect(admin_url('clients/client/' . $id . '?group=contacts&new_contact_only=true'));                        
                    } else {
                        $this->contact_new($id);
                        redirect(admin_url('clients/client/' . $id));
                    }			
    ...
    public function contact_new($customer_id, $contact_id = '', $save_contact_only = '0')
    {
        if (!has_permission('customers', '', 'view')) {
        ...
        if ($this->input->post()) {
            ...
            if(isset($save_contact_only) && $data['save_contact_only']==1){
                //$save_contact_only=true;
                if($data['email']==''){
                    $data['email']="email".time().'@signatureimpactwindows.net';
                }
                if($data['password']==''){
            ...
            unset($data['contactid']);
            unset($data['contact-autofill']);
            unset($data['save_contact_only']);
            if ($contact_id == '') {
                if (!has_permission('customers', '', 'create')) {
                    if (!is_customer_admin($customer_id)) {
        ...
        if ($contact_id == '' && $save_contact_only==1) {
            $title = "Add New Contact Only";            
        } elseif ($contact_id == '') {
        ...
        $data['client']                = $client;
        $data['save_contact_only']=$save_contact_only;
        $this->load->view('admin/clients/modals/contact', $data);
    }



    public function contact($customer_id, $contact_id = '', $save_contact_only = '0')
    {
        if (!has_permission('customers', '', 'view')) {
            if (!is_customer_admin($customer_id)) {
                echo _l('access_denied');
