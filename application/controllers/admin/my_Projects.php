...
class Projects extends Admin_controller
{
    ...
    public function add_expense()
    {
     ...
     public function email_send(){
            ...
            $data['email_number'] = $this->input->post('email_number', false);
             $data['project_id'] = $this->input->post('project_id', false);

	$this->load->library('email', $config);
	$subject="Thank You for choosing Signature Impact Windows & Doors";
	if($data['email_number']=="10"){
		$body = $this->load->view('admin/Welcome/first.html','',TRUE);$subject="Email";

	}
	if($data['email_number']=="2"){
		$body = $this->load->view('admin/Welcome/permit.html','',TRUE);$subject="Your permit has been submitted";

	}
	if($data['email_number']=="3"){
		$body = $this->load->view('admin/Welcome/Order-Processing.html','',TRUE);$subject="Your order has been placed";
	}
   if($data['email_number']=="4"){
		$body = $this->load->view('admin/Welcome/City-Approved.html','',TRUE);$subject="The city has approved you project";

	}
	if($data['email_number']=="5"){
		$body = $this->load->view('admin/Welcome/Order-Arrived.html','',TRUE);$subject="Your order has arrived";

	}
	if($data['email_number']=="6"){
		$body = $this->load->view('admin/Welcome/Order-Completed.html','',TRUE);$subject="Your order has been completed";

	}
	//end here
    $customer_email=$this->projects_model->email_get($data['project_id']);
  
$this->load->library('email');
$this->email->set_newline("\r\n");
$this->email->from("noreply@signatureimpactwindows.net", 'Signature Impact Windows & Doors');
...
//$mail->AddAddress("email@gmail.com");
   ...
   // mail($customer_email, $subject ,   $body,$headers );
	//$mail = $smtp->send($customer_email, $headers, $body);
  $result= $this->projects_model->email_update( $data['email_number'],  $data['project_id']);
        ...
        if ($this->input->post()) {
            ...
            if ($id == '') {
                ...
                if ($id) {
                    ...
                    //welcome email template goes here
                   $headers  = 'MIME-Version: 1.0' . "\r\n";
					$headers .= 'Content-type: text/html; charset=iso-8859-1' . "\r\n";
						$this->load->library('email', $config);
                	    	            $customer_email=$this->projects_model->email_get($id);
                	    	            $body = $this->load->view('admin/Welcome/first.html','',TRUE);
...
$this->email->subject("Thank You for choosing Signature Impact Windows & Doors");
$this->email->message($body);
$this->email->send();
 					
                //   mail($customer_email, "Welcome Email" ,$body,$headers);
