...
{
    $CI = & get_instance();
    $CI->db->select('date,number,prefix,number_format,woocommerce_order_id')->from('tblinvoices')->where('id', $id);
    ...
    $date          = $invoice->date;
    $woocommerce_order_id          = $invoice->woocommerce_order_id;
    ...
    if ($format == 1) {
        ...
        // Number-woo-commerce
        $number = $prefix .'-00'. $woocommerce_order_id;
        // $number = $prefix .'-'. str_pad($woocommerce_order_id, $prefixPadding, '0', STR_PAD_LEFT);
    }
