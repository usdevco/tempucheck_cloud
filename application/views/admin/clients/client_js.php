...
$(function() {
    ...
    if (get_url_param('new_contact_only')) {
        contact_only(customer_id);
    }	
    ...
    $('.customer-form-submiter').on('click', function() {
        ...
        if (form.valid()) {
            ...
            } else if ($(this).hasClass('save-and-add-contact-only')) {
                form.find('.additional').html(hidden_input('save_and_add_contact_only', 'true'));
            } else {
...
function validate_contact_only_form() {
    _validate_form('#contact-form', {
...
function contact_only(client_id, contact_id) {
    if (typeof(contact_id) == 'undefined') {
        contact_id = '0';
    }
	var save_contact_only=1; 
    requestGet('clients/contact/' + client_id + '/' + contact_id+ '/' + save_contact_only).done(function(response) {
        $('#contact_data').html(response);
        $('#contact').modal({
            show: true,
            backdrop: 'static'
