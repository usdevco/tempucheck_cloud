  ...
  <div id="invoice-preview">
   ...
   <?php if($invoice->project_id != 0){
    $invoice_project_data_name = isset($invoice->project_data->name) ? $invoice->project_data->name : NULL;
    ?>
   <div class="col-md-12">
      <h4 class="font-medium mtop15 mbot20"><?php echo _l('related_to_project',array(
         _l('invoice_lowercase'),
         _l('project_lowercase'),
         '<a href="'.admin_url('projects/view/'.$invoice->project_id).'" target="_blank">' . $invoice_project_data_name . '</a>',
         )); ?></h4>
      </div>
      <?php } ?>
   ...
   <?php } ?>
   <hr />
   <?php if($invoice->clientnote != '' && $invoice->woo_commerce !='WOO-COMMERCE'){ ?>
