...
$aColumns = [
    ...
    'total_tax',
    'YEAR(date) as year',
    'tblinvoices.datecreated as added_invoice_on',
    'date',
...
foreach ($rResult as $aRow) {
   ...
   { 
        $accessToken = isset($_SESSION['sessionAccessToken']) && $_SESSION['sessionAccessToken'] ? $_SESSION['sessionAccessToken'] : false;



        if($aRow['qb_invoice_id'])
        {
            $qb_connect_class = $accessToken ? ' qb_exported' : ' connect_to_qb_btn';
            $qb_delete_class = $accessToken ? ' qb_delete_exported' : ' connect_to_qb_btn';
            $row[] = "<a href='javascript:void(0)' disabled class='btn btn-sm btn-success ". $qb_connect_class."' data-id='".$aRow['id']."'>QB</a>

            <a href='javascript:void(0)' class='btn btn-sm btn-danger ". $qb_delete_class."' data-qb_invoice_id='".$aRow['qb_invoice_id']."' data-id='".$aRow['id']."'><i class='fa fa-trash'></i></a>

            ";
        }
        else
        {
            $qb_connect_class = $accessToken ? ' record_in_qb_btn' :  ' connect_to_qb_btn';
            $qb_delete_class = $accessToken ? ' qb_not_exported' : ' connect_to_qb_btn';

            $row[] = "<a href='javascript:void(0)' class='btn btn-sm btn-warning ". $qb_connect_class."' data-id='".$aRow['id']."'>QB</a>

             <a href='javascript:void(0)' disabled class='btn btn-sm btn-danger ". $qb_delete_class."' data-qb_invoice_id='".$aRow['qb_invoice_id']."' data-id='".$aRow['id']."'><i class='fa fa-trash'></i></a>
